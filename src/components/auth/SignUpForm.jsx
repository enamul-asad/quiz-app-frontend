  ...
  return (
    ...
    `}>
      <form onSubmit={handleSubmit} className="bg-white flex flex-col items-center justify-center h-full px-8 w-full max-w-sm">
        ...
        {success && <div className="bg-green-50 text-green-600 text-xs p-2 rounded w-full mb-3 text-center border border-green-100 font-medium">{success}</div>}
        ...
        <button 
            ...
            className={`bg-[#512da8] text-white text-xs py-3 px-12 rounded-lg font-bold uppercase tracking-wider shadow-lg hover:bg-[#4527a0] mt-4 transition-transform active:scale-95 flex items-center justify-center
        ...
        <div className="mt-8 md:hidden flex flex-col items-center w-full pt-4 border-t border-gray-100">
            ...
            <button type="button" onClick={() => setIsActive(false)} className="text-[#512da8] font-bold text-xs border border-[#512da8] px-6 py-2 rounded-full hover:bg-[#512da8] hover:text-white transition-all">Log In</button>
        </div>
      </form>
    </div>
  );
